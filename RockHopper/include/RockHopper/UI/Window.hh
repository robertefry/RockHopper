...
namespace RockHopper::UI
{
    ...
    public:
        ...
        [[nodiscard]] virtual auto set_visible(Util::In<visible_t> const) -> Util::Future<void> = 0;

        enum focused_t { UNFOCUSED, FOCUSED };
        [[nodiscard]] virtual auto get_focused() const -> focused_t = 0;
        [[nodiscard]] virtual auto set_focused(Util::In<focused_t> const) -> Util::Future<void> = 0;

        struct position_t { int32_t x, y; };
        [[nodiscard]] virtual auto get_position() const -> position_t = 0;
        [[nodiscard]] virtual auto set_position(Util::In<position_t> const) -> Util::Future<void> = 0;

        struct dimension_t { uint32_t wid, hei; };
        [[nodiscard]] virtual auto get_dimension() const -> dimension_t = 0;
        [[nodiscard]] virtual auto set_dimension(Util::In<dimension_t> const) -> Util::Future<void> = 0;

        using title_t = std::string;
        [[nodiscard]] virtual auto get_title() const -> title_t = 0;
        [[nodiscard]] virtual auto set_title(Util::In<title_t> const) -> Util::Future<void> = 0;

        using swap_interval_t = uint32_t;
        [[nodiscard]] virtual auto get_swap_interval() const -> swap_interval_t = 0;
        [[nodiscard]] virtual auto set_swap_interval(Util::In<swap_interval_t> const) -> Util::Future<void> = 0;
